...
export async function setupGmailFilters(prev) {
  ...
  if (projectAction === 'new') {
    ...
    createSpinner.start('建立新 Apps Script 專案')
    try {
      ...
      // clasp create 輸出：Created new standalone script: https://script.google.com/d/<ID>/edit
      const match = output.match(/script\.google\.com\/d\/([^/]+)\/edit/)
      scriptId = match ? match[1] : ''
      createSpinner.stop('Apps Script 專案建立完成')
    } catch (err) {
      createSpinner.stop('建立失敗')
      p.log.warn(`clasp create 失敗：${err.message}`)
      _showManualSteps()
      return null
    }
  } else {
