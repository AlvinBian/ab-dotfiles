...
#
# 用法：
#   pnpm run build           ← 從 ab-dotfiles 自身打包
#   pnpm run build           ← 從任意專案目錄執行，自動整合專案配置
...
print(','.join(stack))
" 2>/dev/null | while IFS=',' read -ra parts; do
      for p in "${parts[@]}"; do [[ -n "$p" ]] && TECH_STACK+=("$p"); done
    done

    # 用 python3 直接輸出並 capture
    DETECTED=$(python3 -c "
import json
...
cat > "$BUILD_DIR/.claude-plugin/plugin.json" << JSON_EOF
{
  ...
  "techStack": $(python3 -c "import json; print(json.dumps(${TECH_STACK[@]@Q} if '${TECH_STACK[*]}' else []))" 2>/dev/null || echo "[]")
}
JSON_EOF
