...
function saveSessionSync(data) {
  ...
  fs.writeFileSync(tmpPath, JSON.stringify({ ...data, timestamp: new Date().toISOString() }, null, 2) + '\n', 'utf8')
...
export function saveSession(data) {
  _writeQueue = _writeQueue.then(() => {
    ...
    } catch { /* session 是 best-effort，寫入失敗不應中斷安裝流程 */ }
  })
  return _writeQueue
}
...
export async function updateSessionProgress(progress) {
  return _writeQueue = _writeQueue.then(() => {
    try {
