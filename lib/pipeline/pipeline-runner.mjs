...
import { AI_CONCURRENCY, AI_ECC_TIMEOUT, GH_CONCURRENCY, GH_REPO_ANALYZE_TIMEOUT } from '../core/constants.mjs'
  ...
  const [analysisResults, eccFetchResult] = await Promise.all([
    ...
    (async () => {
      ...
      await pMap(repos, async repo => {
        try {
          const r = await Promise.race([analyzeRepo(repo), new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), GH_REPO_ANALYZE_TIMEOUT))])
        ...
        callClaudeJSON(transPrompt, { model: 'haiku', effort: 'low', timeoutMs: AI_ECC_TIMEOUT, retries: 0 })
          .then(r => {
            if (r?.translations) {
              ...
              let current = {}
              try { current = JSON.parse(fs.readFileSync(transPath, 'utf8')) } catch {}
