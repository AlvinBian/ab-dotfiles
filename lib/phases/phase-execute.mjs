															...
															// [2] 全局配置（settings + hooks dispatch）
															{
																...
																task: (_, subtask) =>
																	subtask.newListr([
																		...
																		{
																			title:
																				"hooks/slack-dispatch.sh — Slack 通知分發器",
																			enabled: () => has("slack"),
																			task: async (_, sub) => {
																				const src = path.join(
																					repoDir,
																				...
																				const repoEnv = path.join(
																					repoDir,
																					".env",
																				);
																				if (fs.existsSync(repoEnv)) {
																					const repoEnvContent =
																						fs.readFileSync(repoEnv, "utf8");
																					const channel =
															...
															// [3] Claude 安裝（commands + agents + rules + hooks → ~/.claude/）
															{
																...
																task: async (_, subtask) => {
																	...
																	if (installSelections.rules?.length)
																		parts.push(
																			`${installSelections.rules.length} rules`,
																		);
																	if (installSelections.hooks?.length)
																		parts.push(
																			`${installSelections.hooks.length} hooks`,
																		);
																		...
																		[
																			{
																				...
																				enabled: () =>
																					plan.targets.includes("claude-dev"),
																				task: async () => {
																					await new Promise(
																						(resolve, reject) => {
																							const child = spawn(
														...
														[
															...
															// [5] CLAUDE.md 生成 → ~/.claude/projects/
															{
																...
																task: async (_, subtask) => {
																	...
																	const result =
																		deployAllProjectClaudeMd(items);
																	const parts = [];
