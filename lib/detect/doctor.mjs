...
function runNvm(nvmCmd) {
  ...
  try {
    execSync(`bash -c "source ${nvmDir}/nvm.sh 2>/dev/null && nvm ${nvmCmd}"`, { stdio: 'inherit', timeout: 120000 })
    return true
  } catch { return false }
}
...
function checkNvm() {
  const nvmDir = process.env.NVM_DIR || `${process.env.HOME}/.nvm`
  try {
    execSync(`bash -c "source ${nvmDir}/nvm.sh 2>/dev/null && nvm --version"`, { stdio: 'pipe', timeout: 3000 })
    return true
  } catch { return false }
}
...
function nvmVersion() {
  const nvmDir = process.env.NVM_DIR || `${process.env.HOME}/.nvm`
  try {
    return execSync(`bash -c "source ${nvmDir}/nvm.sh 2>/dev/null && nvm --version"`, { encoding: 'utf8', stdio: 'pipe', timeout: 3000 }).trim()
  } catch { return null }
}
