...
export async function adjustClaude({ flagAll = false, manual = false } = {}) {
  ...
  if (result) patchSession({ install: { ...(loadSession()?.install || {}), claude: result } })
}

/**
 * 調整 2：重新套用全局設定（settings.json + keybindings.json）
 */
export async function adjustGlobalSettings() {
  const s = p.spinner()
  s.start('套用全局設定...')
  const { deploySettings, deployKeybindings } = await import('../deploy/deploy-global.mjs')
  const settingsTemplate = JSON.parse(
    fs.readFileSync(path.join(REPO, 'claude', 'settings-template.json'), 'utf8')
  )
  const keybindingsTemplate = JSON.parse(
    fs.readFileSync(path.join(REPO, 'claude', 'keybindings-template.json'), 'utf8')
  )
  const settingsResult = deploySettings(settingsTemplate)
  const kbResult = deployKeybindings(keybindingsTemplate)
  s.stop(
    `全局設定已套用  ${pc.dim(`settings +${settingsResult.permissionsAdded} 規則`)}` +
    (kbResult.skipped ? `  ${pc.dim('keybindings 已存在，略過')}` : '')
  )
  patchSession({ adjustedAt: new Date().toISOString() })
}
...
export async function adjustSlack() {
  ...
  if (result) patchSession({ slack: result })
}
...
export async function adjustClaudeMd({ skipConfirm = false } = {}) {
  ...
  patchSession({ claudeMdUpdatedAt: new Date().toISOString() })
}
...
export async function adjustZsh({ flagAll = false, manual = false } = {}) {
  ...
  if (result) patchSession({ install: { ...(loadSession()?.install || {}), modules: result.modules } })
}

/**
 * 調整 6：重新設定 Gmail 5-Tier 分級
 */
export async function adjustGmail() {
  const { setupGmailFilters } = await import('../gmail/gmail-setup.mjs')
  const session = loadSession()
  const result = await setupGmailFilters(session)
  if (result) patchSession({ gmail: result })
}
