...
import { AI_TIMEOUT, AI_MODEL, AI_EFFORT } from '../core/constants.mjs'

const execFileAsync = promisify(execFile)

// ── 共用 CLI flags ──────────────────────────────────────────────
const SYSTEM_PROMPT = 'You are a concise assistant. Only respond to the user prompt directly. No greetings, no extra explanation.'
const BASE_FLAGS = [
...
export async function callClaude(prompt, {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (e.killed) {
        console.error(`  ⏱ Claude CLI 超時（${timeoutMs}ms），${attempt < retries ? '重試中...' : '放棄'}`)
      }
 ...
 */
export function callClaudeJSONStream(prompt, {
  timeoutMs = AI_TIMEOUT,
  model = AI_MODEL,
  effort = AI_EFFORT,
  onProgress = () => {},
  ...
  return new Promise((resolve) => {
    ...
    if (!cli) {
      if (!_cliWarnEmitted) {
        ...
        process.stderr.write('⚠️  claude CLI 未找到，AI 分析將跳過（安裝：https://claude.ai/download）\n')
      }
      resolve(null); return
    }
    ...
    let buffer = ''
    let resultText = null
    const usage = { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheCreateTokens: 0, costUSD: 0, durationMs: 0 }
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (event.type === 'assistant' && event.message?.usage) {
            ...
            usage.cacheCreateTokens = u.cache_creation_input_tokens || usage.cacheCreateTokens
          ...
          if (event.type === 'result') {
            ...
            if (event.modelUsage) {
              ...
              if (m) {
                ...
                usage.cacheReadTokens = m.cacheReadInputTokens || usage.cacheReadTokens
                usage.cacheCreateTokens = m.cacheCreationInputTokens || usage.cacheCreateTokens
