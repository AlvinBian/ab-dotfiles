...
async function main() {
  p.intro(' ab-dotfiles 卸載 ')

  // 統計將刪除的項目
  const stats = {
    commands: ALL_COMMANDS.filter(c => fs.existsSync(path.join(CLAUDE_DIR, 'commands', `${c}.md`))).length,
    agents: ALL_AGENTS.filter(a => fs.existsSync(path.join(CLAUDE_DIR, 'agents', `${a}.md`))).length,
    rules: ALL_RULES.filter(r => fs.existsSync(path.join(CLAUDE_DIR, 'rules', `${r}.md`))).length,
    hooks: fs.existsSync(path.join(CLAUDE_DIR, 'hooks.json')) ? 1 : 0,
    settings: fs.existsSync(path.join(CLAUDE_DIR, 'settings.json')) ? 1 : 0,
    keybindings: fs.existsSync(path.join(CLAUDE_DIR, 'keybindings.json')) ? 1 : 0,
    dispatch: fs.existsSync(path.join(CLAUDE_DIR, 'hooks', 'slack-dispatch.sh')) ? 1 : 0,
  }
  const total = stats.commands + stats.agents + stats.rules + stats.hooks + stats.settings + stats.keybindings + stats.dispatch
  ...
  if (fs.existsSync(projectsDir)) {
    for (const dir of fs.readdirSync(projectsDir)) {
      const mdPath = path.join(projectsDir, dir, 'CLAUDE.md')
      if (fs.existsSync(mdPath)) {
        const content = fs.readFileSync(mdPath, 'utf8')
        if (content.startsWith('# Generated by ab-dotfiles')) claudeMdCount++
      }
    }
  }

  if (total === 0 && claudeMdCount === 0) {
    p.log.info('沒有找到 ab-dotfiles 管理的配置')
    p.outro()
    return
  }
...
完全恢復到 setup 前：pnpm run restore-original`)
  ...
  if (fs.existsSync(dispatchPath)) { fs.unlinkSync(dispatchPath); removed++ }

  // 清除 ab-dotfiles 生成的 CLAUDE.md
  if (fs.existsSync(projectsDir)) {
    for (const dir of fs.readdirSync(projectsDir)) {
      const mdPath = path.join(projectsDir, dir, 'CLAUDE.md')
      if (fs.existsSync(mdPath)) {
        const content = fs.readFileSync(mdPath, 'utf8')
        if (content.startsWith('# Generated by ab-dotfiles')) {
