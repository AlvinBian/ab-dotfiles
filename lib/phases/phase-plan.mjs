...
export async function phasePlan(plan) {
  ...
  // 列表項 bullet 展示
  const bullet = (items, indent = '      ') => items.map(i => `${indent}· ${i}`)
  ...
  // 1. Repos
  lines.push(`1. Repos（${plan.mainCount} ⭐主力 · ${plan.tempCount} 🔄臨時）`)
  for (const r of plan.repos) {
    const icon = r.role === 'main' ? '⭐' : '🔄'
  ...
  lines.push(...bullet(g.commands))
  lines.push(`   2.2 Agents（${g.agents.length}）`)
  lines.push(...bullet(g.agents))
  lines.push(`   2.3 Rules（${g.rules.length}）`)
  lines.push(...bullet(g.rules))
  ...
  if (plan.projects.length > 0) {
    ...
    if (mainPrj.length) {
      ...
      lines.push(...bullet(mainPrj.map(p => p.repo.split('/')[1])))
    }
    if (tempPrj.length) {
      lines.push(`   3.2 靜態模板（${tempPrj.length} 個🔄臨時）`)
      lines.push(...bullet(tempPrj.map(p => p.repo.split('/')[1])))
    }
  }
