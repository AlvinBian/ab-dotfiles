...
export async function collectFullStatus() {
  ...
  // ECC 資源
  const eccDir = path.join(process.cwd(), 'ecc', 'everything-claude-code')
  const ecc = {
  ...
  const templateSettingsPath = path.join(process.cwd(), 'claude', 'settings-template.json')
  ...
  // 磁碟佔用
  const diskUsage = {
    cache: dirSize(path.join(process.cwd(), '.cache')),
  ...
  // ZSH 模組
  const zshModulesDir = path.join(process.cwd(), 'zsh', 'modules')
