 ...
 */
export async function groupedMultiselectWithAll({ message, groups, required = false, initialValues = [] }) {
  const result = await handleCancel(await p.groupMultiselect({
...
export async function smartSelect({
  ...
  // 調整模式 — 有分組用 groupedMultiselectWithAll，否則用 multiselectWithAll
  if (groups) {
    result = await groupedMultiselectWithAll({ message: title, groups, initialValues: effectivePreselected, required })
  } else {
