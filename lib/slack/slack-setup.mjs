...
export async function setupSlackNotify(prev) {
  ...
  const currentChannelName = env('SLACK_NOTIFY_CHANNEL_NAME', '') || prev?.slackChannelName || ''
  const currentWorkspaceUrl = env('SLACK_WORKSPACE_URL', '') || prev?.slackWorkspaceUrl || ''
  ...
  if (prev && currentChannel && currentMode) {
    ...
    const channelHref = currentWorkspaceUrl ? `${currentWorkspaceUrl}/archives/${currentChannel}` : ''
    const displayLabel = currentMode === 'channel'
      ? `${hyperlink(`#${channelLabel}`, channelHref)} (${currentChannel})`
      : currentMode === 'dm' ? 'DM 私發' : '已關閉'
    const keep = handleCancel(await p.confirm({
      message: `Slack 通知：${displayLabel}，保持不變？`,
      initialValue: true,
    }))
    if (keep === true) {
      const wsUrl = env('SLACK_WORKSPACE_URL', '')
      return {
        channelId: currentChannel,
        channelName: currentChannelName,
        channelUrl: wsUrl ? `${wsUrl}/archives/${currentChannel}` : '',
        mode: currentMode,
      }
    }
  }
  ...
  if (action === 'dm') {
    ...
    p.log.success(`已設定 DM 模式${userId ? `（${userId}）` : '（首次發送時自動查詢 User ID）'}`)
  ...
  if (hasChannel === 'create') {
    try {
      ...
      execFileSync('pbcopy', [], { input: channelName, stdio: ['pipe', 'pipe', 'pipe'] })
  ...
  const workspaceUrl = manualInput.match(/^(https:\/\/[^/]+)/)?.[1] || env('SLACK_WORKSPACE_URL', '')
  const channelUrl = workspaceUrl ? `${workspaceUrl}/archives/${channelId}` : ''
  ...
  return { channelId, channelName, channelUrl, userId, mode: 'channel' }
}
