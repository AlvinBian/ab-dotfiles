 ...
 *
 * 快取策略：
 *   dist/cache/sources/{source-name}/
...
const LANG_PREFIXES = [...new Set(Object.values(TECH_TO_LANG))].map(l => `${l}-`).concat(['cpp-', 'go-'])

// ── 快取管理 ──────────────────────────────────────────────────────

/**
 * 取得 source 的快取目錄路徑
 * @param {string} cacheBase - 快取根目錄（dist/cache/sources/）
...
export async function fetchAllSources(sources, techStacks, localDir, onProgress = () => {}) {
  ...
  const cacheBase = path.join(localDir, 'dist', 'cache', 'sources')
  ...
  for (const source of sorted) {
    ...
    results.push({
      name: source.name,
      repo: source.repo,
      version: typeof sha === 'string' ? sha.slice(0, 8) : 'cached',
