...
const INFRA_SIGNALS = /^(docker-compose|Dockerfile|nginx|Makefile|Vagrantfile|Procfile|serverless|terraform|pulumi|k8s|kubernetes|helm)/i
const INFRA_DIRS = /^(nginx|redis|postgres|mysql|mongo|pgadmin|jaeger|grafana|prometheus|elasticsearch|rabbitmq|kafka|certificates|ssl)/i

/**
 * 從 analyzeRepo 結果提取結構化摘要
 */
export function buildRepoSummary(repoName, analysis) {
  const { context, languages, description, stars, topics, rootFiles, skills } = analysis
  const meta = { description, stars, topics, languages: Object.keys(languages), rootFiles: rootFiles || [] }
  const tf = context.techFiles

  const deps = {}, devDeps = {}, phpDeps = {}
  for (const [fileName, content] of Object.entries(tf)) {
    if (fileName.endsWith('package.json')) {
      try {
        const pkg = JSON.parse(content)
        for (const [n, v] of Object.entries(pkg.dependencies || {})) { if (!n.startsWith('@types/')) deps[n] = v }
        for (const [n, v] of Object.entries(pkg.devDependencies || {})) { if (!n.startsWith('@types/')) devDeps[n] = v }
      } catch {}
    }
    if (fileName.endsWith('composer.json')) {
      try {
        const c = JSON.parse(content)
        for (const [n, v] of Object.entries(c.require || {})) { if (!/^(php$|ext-|lib-|composer\/|psr\/)/.test(n)) phpDeps[n] = v }
      } catch {}
    }
  }
  ...
  if (npmDevDeps.length) parts.push(`  devDependencies: ${npmDevDeps.join(', ')}`)
  if (phpDepNames.length) parts.push(`  composer require: ${phpDepNames.join(', ')}`)
  ...
  if (classified.size > 0) {
    const preParts = [...classified.entries()].map(([cat, pkgs]) => `${cat}: ${pkgs.slice(0, 3).join(', ')}${pkgs.length > 3 ? ' …' : ''}`)
    parts.push(`  已知分類: ${preParts.join(' | ')}`)
  }
  if (unclassified.length > 0) {
    parts.push(`  待分類: ${unclassified.slice(0, 10).join(', ')}${unclassified.length > 10 ? ' …' : ''}`)
  }

  if (!npmDeps.length && !npmDevDeps.length && !phpDepNames.length) {
    const dirs = meta.rootFiles.filter(f => !f.startsWith('.') && !f.includes('.')).slice(0, 15)
  ...
  return {
    summary: parts.join('\n'),
    meta,
    npmDeps: new Set([...Object.keys(deps), ...Object.keys(devDeps)].filter(n => !n.startsWith('@types/'))),
 ...
 */
export async function classifyRepo(repoName, summary, {
  baseDir,
  model = 'haiku',
  effort = 'low',
  timeoutMs = 30000,
...
回傳純 JSON：{"techStacks":{"分類":["id"]},"coreCategories":["核心分類名"],"reasoning":"一句話說明主要技術方向"}`
  ...
  if (result?.techStacks && cacheEnabled && baseDir) {
    writeCache(baseDir, 'repo-ai', cacheKey, { techStacks: output.techStacks, coreCategories: output.coreCategories, reasoning: output.reasoning, tokens })
  }
