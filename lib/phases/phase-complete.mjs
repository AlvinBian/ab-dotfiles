...
import { buildDescriptionCache } from '../descriptions.mjs'
import { notifyComplete } from '../slack-notify.mjs'
...
export async function phaseComplete(plan, {
  ...
  const claudeMdCount = plan.projects?.filter(p => p.localPath).length || 0
  ...
  const descCount = buildDescriptionCache(claudeDir)
