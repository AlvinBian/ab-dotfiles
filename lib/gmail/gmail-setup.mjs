...
export async function setupGmailFilters(prev) {
  ...
  try {
    ...
    p.log.info(`clasp 已安裝（${claspPath}）`)
  } catch {
    p.log.warn('clasp 未安裝')
  ...
  // Step 4: 自動偵測 — 有 session scriptId 就複用，否則建新專案
  let scriptId = prev?.gmail?.scriptId || ''

  if (scriptId) {
    p.log.info(`偵測到現有專案（${scriptId}），更新腳本`)
  ...
  const pushResult = await _exec(claspPath, ['push', '--force'], tmpDir)
  if (pushResult.code !== 0) {
    const pushMsg = pushResult.stderr || pushResult.stdout || ''
    pushSpinner.stop(`腳本推送失敗${pushMsg ? '：' + pushMsg.split('\n')[0].slice(0, 60) : ''}`)
    _showManualSteps()
    return null
  }
  ...
  p.log.info(`請在 Apps Script 編輯器中完成最後一步：
  1. 上方函式選擇器選 ${pc.cyan('setupAndApply')}（建立 filter + 追溯分類已有郵件）
  2. 點擊 ▶ 執行
  3. 首次執行需授權 → 「審查權限」→ 「進階」→ 「允許」

  執行完成後，開啟 Gmail：
  • 左側欄點 ${pc.red('action/required')} → 查看所有重要郵件
  • 切換優先收件匣：⚙️ → 收件匣類型 → ${pc.cyan('優先收件匣')}
    重要郵件會自動顯示在收件匣頂部獨立區塊`)
  try { execFileSync('open', [scriptUrl]) } catch { /* ignore */ }
...
async function _claspCreateWithRetry(claspPath, tmpDir) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const { code, stdout, stderr } = await _exec(claspPath, ['create', '--type', 'standalone', '--title', 'Gmail 5-Tier Filters', '--rootDir', tmpDir], tmpDir)
    if (code === 0) {
      const match = stdout.match(/script\.google\.com\/d\/([^/]+)\/edit/)
      s.stop('Apps Script 專案建立完成')
      return match ? match[1] : ''
    } else {
      const msg = stderr || stdout || ''
      s.stop(`Apps Script 專案建立失敗${msg ? '：' + msg.split('\n')[0].slice(0, 60) : ''}`)
      if (msg.includes('Apps Script API') && attempt < maxRetries) {
        ...
        p.log.info('正在重試建立 Apps Script 專案...')
        continue
      }
...
/** 非阻塞執行指令（讓 spinner 能正常旋轉） */
function _exec(bin, args, cwd) {
  return new Promise(resolve => {
    let stdout = '', stderr = ''
...
function _showManualSteps() {
  ...
  5. clasp push
  6. 在 Apps Script 編輯器執行 setupAndApply（建立 filter + 追溯分類）`)
}
