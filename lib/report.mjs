...
function renderEcc(ecc) {
  ...
  return section('Source 融合', '<p class="section-desc">ECC（Everything Claude Code）外部社群資源。「新增」表示本地沒有的項目已融合，「跳過」表示本地已有同名項目優先保留。</p>' + inner)
}
...
function renderInstalled(installed) {
  ...
  return section('已安裝項目', '<p class="section-desc">以下配置已安裝到 ~/.claude/ 目錄，對所有專案全局生效。帶描述的項目來自 ab-dotfiles，無描述的可能是 ECC 外部資源。</p>' + inner)
}
...
function renderAuditTrail(auditSummary) {
  ...
  return section('決策審計鏈', '<p class="section-desc">Pipeline 分析過程中的每一步決策記錄，包括 AI 分類結果、整合去重判斷、ECC 匹配邏輯。用於追溯為什麼某個技術棧被選中或排除。</p>' + `<table><tbody>${rows}</tbody></table>`)
}

function renderBackup(backupDir) {
  if (!backupDir) return ''
  return section('備份', '<p class="section-desc">每次安裝前自動備份現有配置，保留最近 10 次。可用 pnpm run restore 選擇備份版本還原。</p>' + `<p class="mono">${esc(backupDir)}</p>`)
}
...
<div id="tab-overview" class="tab-content active">
  ...
  <div class="card">
    ...
    <p class="section-desc">左圖：AI 辨識的技術棧按分類統計（點擊柱狀可跳轉到 Repos Tab 篩選）。右圖：已安裝的配置項目類型分佈。</p>
    <div class="chart-row">
...
function renderTabTechStacks(data) {
  ...
  for (const [cat, items] of Object.entries(techStacks)) {
    if (!items?.length) continue
    techStacksHtml += `<div class="group-label">${esc(cat)}</div><div>${items.map(t => badgeWithDesc(t, 'blue', null, null)).join('')}</div>`
  }

  return `
<div id="tab-stacks" class="tab-content">
  ${techStacksHtml ? `<div class="card"><h2 class="section-title">AI 辨識技術棧</h2><p class="section-desc">AI 分析所有選定 repos 的 package.json、語言特徵和配置檔案，自動辨識使用的技術棧並分類。點擊分類可跳轉到 Repos Tab 篩選。</p>${techStacksHtml}</div>` : ''}
  ${hasHeatmap ? `
  <div class="card">
    <h2 class="section-title">Repo × 技術棧矩陣熱力圖</h2>
    <p class="section-desc">顏色深度表示每個 repo 在該技術分類下使用的技術數量。Hover 可查看具體技術名稱。</p>
...
function renderTabRepos(data) {
  ...
  // 用 data.repos 順序（已按 ⭐主力 → 🔄臨時 → 🔧工具 排序）
  ...
  const cards = repoKeys.map(repo => {
    ...
    for (const [cat, techs] of Object.entries(repoData.stacks || {})) {
      ...
      stackBadges += `<div style="margin-top:6px"><span style="font-size:.78rem;color:#8b949e;margin-right:4px">${esc(cat)}:</span>${techs.map(t => { const d = getDescription(t); return d ? badge(t, 'blue', d) : badge(t, 'blue') }).join('')}</div>`
    }
    const roleInfo = data.repoRoles?.[repo]
    const roleLabel = roleInfo?.role === 'main' ? '⭐ 主力' : roleInfo?.role === 'tool' ? '🔧 工具' : '🔄 臨時'
    const roleBadge = roleInfo ? badge(roleLabel, roleInfo.role === 'main' ? 'green' : 'grey') : ''
...
<div id="tab-repos" class="tab-content">
  <div class="card" style="margin-bottom:16px">
    <p class="section-desc" style="margin:0">每個 Repo 卡片顯示 AI 分析的摘要和偵測到的技術棧。使用搜尋框過濾，或從技術棧 Tab 的圖表點擊分類來篩選。</p>
  </div>
...
function renderTabProjects(data) {
  const projects = data.projects || []
  const roles = data.repoRoles || {}
  if (!projects.length && !Object.keys(roles).length) {
    return '<div id="tab-projects" class="tab-content"><p style="color:#8b949e">無專案配置資料</p></div>'
  }
  ...
  for (const repo of allRepos) {
    const info = roles[repo] || {}
    const proj = projects.find(p => p.repo === repo)
    const role = info.role || 'temp'
    const localPath = info.localPath ? info.localPath.replace(process.env.HOME, '~') : '未找到'
    const claudeMd = proj ? typeLabel[proj.claudeMdType] || '—' : '—'

    cards += `<div class="repo-card">
      <div class="name">${badge(roleIcon[role] || role, role === 'main' ? 'green' : 'grey')} ${esc(repo)}</div>
      <table style="margin-top:8px">
        <tr><td style="color:#8b949e;width:100px">本機路徑</td><td class="mono">${esc(localPath)}</td></tr>
        <tr><td style="color:#8b949e">CLAUDE.md</td><td>${esc(claudeMd)}</td></tr>
        <tr><td style="color:#8b949e">角色說明</td><td>${role === 'main' ? '完整 AI 分析 + 技術棧上下文 + 開發規範' : role === 'tool' ? '最小配置，只有基本描述' : '精簡模板，快速上手指引'}</td></tr>
      </table>
    </div>`
  }
...
<div id="tab-projects" class="tab-content">
  <div class="card"><h2 class="section-title">專案配置（${allRepos.length} 個 Repos）</h2><p class="section-desc">每個 Repo 根據角色（⭐主力/🔄臨時/🔧工具）生成不同深度的 CLAUDE.md 到 ~/.claude/projects/。⭐主力使用 AI 生成完整技術棧上下文，🔄臨時使用靜態精簡模板。本機路徑由 fd + git remote 自動偵測。</p>${cards}</div>
</div>`
}
