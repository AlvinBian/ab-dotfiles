...
export async function phasePlan(plan) {
  ...
  // 長列表自動換行（每行最多 maxLen 字元）
  const MAX_LINE = 65
  function wrapItems(prefix, items, sep = '、') {
    const result = []
    let line = prefix
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      const next = i === 0 ? item : `${sep}${item}`
      if (line.length + next.length > MAX_LINE && line !== prefix) {
        result.push(line)
        line = '     ' + item  // 續行縮排
      } else {
        line += next
  ...
  // 組裝計畫展示內容 — p.note 內不使用 ANSI 色碼
  const lines = []

  // 現有狀態（只顯示數量，不列名稱——可能有大量 ECC 項目）
  ...
  if (existTotal > 0) {
    ...
    lines.push(`📋 現有：${parts.join(' · ')}${extras.length ? ` · ${extras.join(' · ')}` : ''}`)
    lines.push('')
  }
  ...
  for (const r of plan.repos) {
    ...
    const localPath = r.localPath ? `→ ~/${path.relative(HOME, r.localPath)}` : '⚠ 未找到'
    lines.push(`   ${icon} ${r.fullName.split('/')[1]}  ${localPath}  [${config}]`)
  }
  lines.push('')

  // 全局配置 — 使用 wrapItems 控制行寬
  const g = plan.global
  lines.push('🛠 將安裝')
  if (g.commands.length) lines.push(...wrapItems(`   Commands（${g.commands.length}）：`, g.commands))
  if (g.agents.length) lines.push(...wrapItems(`   Agents（${g.agents.length}）：`, g.agents))
  if (g.rules.length) lines.push(...wrapItems(`   Rules（${g.rules.length}）：`, g.rules))
  if (g.hooks.length) lines.push(`   Hooks（${g.hooks.length}）`)
  lines.push(`   Permission（${g.permissions.allow.length} allow · ${g.permissions.deny.length} deny）`)
  lines.push(`   快捷鍵（${Object.keys(g.keybindings).length}）· AutoMemory · ${g.settings.model}`)
  lines.push('')

  // 專案 CLAUDE.md
  if (plan.projects.length > 0) {
    const mainCount = plan.projects.filter(proj => proj.claudeMdType === 'full').length
    const tempCount = plan.projects.filter(proj => proj.claudeMdType === 'concise').length
    const parts = []
    if (mainCount) parts.push(`${mainCount} AI 生成`)
    if (tempCount) parts.push(`${tempCount} 靜態模板`)
    lines.push(`📂 CLAUDE.md（${parts.join(' + ')}）→ ~/.claude/projects/`)
    if (plan.techStacks.length) {
      lines.push(...wrapItems(`   Stacks（${plan.techStacks.length}）：`, plan.techStacks))
    }
    lines.push('')
  }

  // ECC + zsh
  if (plan.ecc.length > 0) lines.push(`🌐 ECC（${plan.ecc.length} 個外部資源）`)
  if (plan.zshModules.length > 0) lines.push(...wrapItems(`🐚 zsh（${plan.zshModules.length}）：`, plan.zshModules))
  if (plan.ecc.length > 0 || plan.zshModules.length > 0) lines.push('')
  ...
  lines.push(`🔄 變更：${changes.join(' · ')}`)
  lines.push(`💰 AI 費用：~$${plan.aiCost.total.toFixed(2)}`)
