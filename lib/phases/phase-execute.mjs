...
import { deploySettings, deployKeybindings } from '../deploy/deploy-global.mjs'
...
export async function phaseExecute(plan, {
  ...
  const tasks = new Listr([
    // [1/8] 備份現有配置
    {
      ...
      task: async (ctx, task) => {
        const backupTasks = []
        const needsClaude = plan.targets.includes('claude-dev') || plan.targets.includes('slack')
        const needsZsh = plan.targets.includes('zsh')
    ...
    {
      title: '[2/8] 全局配置 → ~/.claude/settings.json + keybindings.json',
      task: (ctx, task) => task.newListr([
        ...
        {
          title: 'keybindings.json — Ctrl+R/T/P/B 快捷鍵',
          task: async (_, subtask) => {
            const templatePath = path.join(repoDir, 'claude', 'keybindings-template.json')
            if (fs.existsSync(templatePath)) {
              const template = JSON.parse(fs.readFileSync(templatePath, 'utf8'))
              const result = deployKeybindings(template)
              if (result.skipped) {
                subtask.skip(result.reason)
              } else {
                subtask.output = '已寫入 5 組快捷鍵'
              }
            }
          },
        },
    ...
    {
      title: `[4/8] ECC（${plan.ecc.length} 個外部資源）+ Stacks（${plan.techStacks.length} 個技術棧）`,
      task: (ctx, task) => task.newListr([
        {
        ...
        {
          title: `Stacks 生成 — ${plan.techStacks.join(', ') || '無'}`,
          task: async (_, subtask) => {
    ...
    {
      title: `[5/8] CLAUDE.md → ~/.claude/projects/（${plan.projects.length} 個 repo）`,
      enabled: () => plan.projects.length > 0 && !isManual,
      task: async (ctx, task) => {
        const items = []
        for (const proj of plan.projects) {
          const perRepo = pipelineResult?.perRepo?.get?.(proj.repo)
        ...
        const parts = []
        if (result.deployed.length) parts.push(`${result.deployed.length} 個已生成`)
        if (result.skipped.length) parts.push(`${result.skipped.length} 個跳過`)
        task.output = parts.join('，') || '無需生成'
      },
    },
    ...
    {
      title: `[7/8] zsh 模組（${plan.zshModules.length} 個）→ ~/.zsh/modules/`,
      enabled: () => plan.targets.includes('zsh') && plan.zshModules.length > 0,
      task: async (ctx, task) => {
        if (targets.zsh) {
