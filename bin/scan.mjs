...
function getRepos() {
	...
	if (fs.existsSync(cacheRepos)) {
		return JSON.parse(fs.readFileSync(cacheRepos, "utf8")).map(
			(e) => parseRepoEntry(e).repo,
		);
	}
...
async function main() {
	...
	for (const r of analysisResults) {
		if (r.status === "fulfilled") {
			...
			results.push(data);
			console.log(
				`  ✔ ${data.name.padEnd(30)} ${data.techs.join(", ") || "(none)"}`,
			);
		} else {
			results.push({
				repo: "?",
	...
	if (!onlySkills) {
		...
		console.log(`\n${"═".repeat(60)}`);
		console.log(
			`📊 掃描報告（${results.length} repos → ${filteredTechs.size} 技術棧）`,
		);
