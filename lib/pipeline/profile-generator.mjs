...
export function showProfile(profile, p) {
  ...
  if (profile.coreSkills?.length) {
    lines.push('')
    lines.push(`${pc.dim('核心技能:')} ${profile.coreSkills.join(' / ')}`)
  }

  if (profile.tags?.length) {
    lines.push(pc.dim('+ ' + profile.tags.join(' · ')))
  }

  if (profile.summary) {
    lines.push('')
    lines.push(pc.cyan(profile.summary))
  }

  lines.push('')
  lines.push(pc.dim('即將根據你的技術棧，打造專屬的 Claude Code 技能庫'))
