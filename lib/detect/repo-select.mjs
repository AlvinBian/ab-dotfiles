...
export async function interactiveRepoSelect(session = null) {
  ...
  // 分組 multiselect 的 groups 結構（僅多 org 時啟用）
  // key = "1. OrgName"（顯示為 group header），value = repo 選項陣列（label 帶 1.1 編號）
  const orgGroups = multiOrg
    ? Object.fromEntries(
        repoOrgs.map((org, oi) => {
          ...
          return [
            ...
            orgRepos.map((r, ri) => {
              ...
              if (r.stars > 0) parts.push(`★${r.stars}`)
              if (r.desc) parts.push(r.desc.slice(0, 30))
              return { value: r.fullName, label: `${oi + 1}.${ri + 1} ${name}`, hint: parts.join(' · ') }
            }),
          ]
        })
      )
  ...
  const showSummary = (pre) => {
    ...
    const grouped = {}
    for (const r of preRepos) {
      const org = r.fullName.split('/')[0]
      if (!grouped[org]) grouped[org] = []
      grouped[org].push(r)
    }
    const lines = []
    repoOrgs.filter(o => grouped[o]).forEach((org, oi) => {
      lines.push(`  ${pc.bold(`${oi + 1}. ${org}`)}`)
      grouped[org].forEach((r, ri) => {
        const name = r.fullName.split('/')[1]
        const parts = []
        if (r.pct > 0) parts.push(`${r.pct}%`)
        if (r.commits > 0) parts.push(`${r.commits} commits`)
        ...
        lines.push(`    ${pc.dim(`${oi + 1}.${ri + 1}`)} ${name}  ${pc.dim(parts.join(' · '))}`)
      })
    })
