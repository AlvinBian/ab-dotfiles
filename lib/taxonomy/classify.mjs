...
const CACHE_DIR = path.join(REPO_ROOT, '.cache', 'taxonomy')
...
function loadData() {
  ...
  // 生成的 JSON 在 .cache/taxonomy/，不存在時自動 build
  if (!fs.existsSync(path.join(CACHE_DIR, 'node-packages.json'))) {
    execSync('node lib/taxonomy/build.mjs', { cwd: REPO_ROOT, stdio: 'inherit' })
  }

  _nodePackages = JSON.parse(fs.readFileSync(path.join(CACHE_DIR, 'node-packages.json'), 'utf8'))
  _phpPackages = JSON.parse(fs.readFileSync(path.join(CACHE_DIR, 'php-packages.json'), 'utf8'))
