...
import {
  GLOBAL_COMMANDS, GLOBAL_AGENTS, GLOBAL_RULES,
  determineRole, getProjectConfig,
 ...
 * @returns {Object} plan
 */
export function generateInstallPlan({ repos, pipelineResult, eccResult, localPaths, profile }) {
  ...
  // 專案配置
  const projects = reposWithRoles.map(r => ({
    repo: r.fullName,
    role: r.role,
    localPath: r.localPath,
    config: getProjectConfig(r.role, pipelineResult?.detectedSkills || []),
  }))
