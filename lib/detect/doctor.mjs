 ...
 * 檢查指定命令是否存在於 PATH
 *
 * @param {string} cmd - 命令名稱（例如 'brew', 'node'）
 * @returns {boolean} 命令存在則為 true
 */
function has(cmd) {
 ...
 * @param {string} [flag='--version'] - 版本旗標（部分工具用 '-v'）
 * @returns {string|null} 版本字串第一行，失敗返回 null
 */
...
/**
 * 執行 shell 命令（繼承 stdio，讓輸出直接顯示在終端）
 *
 * @param {string} cmd - 要執行的完整命令
 * @returns {boolean} 執行成功則為 true
 */
function run(cmd) {
 ...
 *
 * @param {string} nvmCmd - nvm 子命令，例如 '--version' 或 'install --lts'
 * @returns {boolean} 執行成功則為 true
 */
function runNvm(nvmCmd) {
  try {
    execSync(`bash -c "source $NVM_DIR/nvm.sh 2>/dev/null && nvm ${nvmCmd}"`, { stdio: 'inherit', timeout: 120000 })
    return true
  } catch { return false }
}
...
export async function ensureEnvironment() {
  // nvm 是 shell function，需透過 bash source nvm.sh 來檢查
  const nvmOk = (() => { try { execSync('bash -c "source $NVM_DIR/nvm.sh 2>/dev/null && nvm --version"', { stdio: 'pipe', timeout: 3000 }); return true } catch { return false } })()
  const nvmVer = (() => { try { return execSync('bash -c "source $NVM_DIR/nvm.sh 2>/dev/null && nvm --version"', { encoding: 'utf8', stdio: 'pipe', timeout: 3000 }).trim() } catch { return null } })()

  const checks = [
    { name: 'Homebrew', ok: has('brew'), ver: ver('brew', '-v')?.match(/[\d.]+/)?.[0], failLabel: '未安裝', actionLabel: '安裝 Homebrew' },
    { name: 'nvm', ok: nvmOk, ver: nvmVer, failLabel: '未安裝', actionLabel: '安裝 nvm' },
  ...
  for (const m of missing) {
    ...
    if (m.name === 'Node.js') {
      ...
      let ok = false
      if (nvmOk) {
        // nvm 是 shell function，必須透過 bash source 執行
