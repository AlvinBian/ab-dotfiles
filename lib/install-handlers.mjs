...
export async function handleInstallClaude(repoDir, previewDir, step, stepLabel, flagAll, manual = false, skillIds = []) {
  ...
  // hooks 選擇（讀取 hooks.json，讓用戶選個別 hook）
      ...
      try {
        ...
        for (const [event, matchers] of Object.entries(hooksData.hooks || {})) {
          for (const m of matchers) {
            ...
            hookOptions.push({
              value: key,
              label: `${desc.label}  ${pc.dim(desc.hint)}`,
              event,
              matcher: m.matcher,
            })
          }
        ...
        if (hookOptions.length > 0 && !flagAll) {
          const chosen = await multiselectWithAll({
            message: `${stepLabel}Hooks（${hookOptions.length} 個）`,
            options: hookOptions,
            initialValues: hookOptions.map(o => o.value),
          })
          ...
          if (chosen.length > 0) {
            ...
            selectedHooks = filteredHooks
            installHooks = true
          }
        } else if (flagAll) {
          selectedHooks = hooksData
          installHooks = true
        }
      } catch {}
    }
  ...
  if (installHooks) {
    const hookCount = selectedHooks ? Object.values(selectedHooks.hooks || {}).reduce((s, m) => s + m.length, 0) : 1
    cmdArgs.push('--hooks')
    total += hookCount
  }
  if (total === 0) return
  ...
  const hooksLabel = installHooks
    ? ` · ${selectedHooks ? Object.values(selectedHooks.hooks || {}).reduce((s, m) => s + m.length, 0) : ''} hooks`
    : ''
  const summaryParts = []
  ...
  if (installHooks && selectedHooks) {
    for (const [event, matchers] of Object.entries(selectedHooks.hooks || {})) {
      for (const m of matchers) {
        const hookDesc = { 'PostToolUse:Edit|Write': '自動格式化', 'PreToolUse:Edit|Write': '檔案保護', 'SessionStart:compact': 'Context 壓縮提示', 'Stop:': '任務完成檢查' }
        const key = `${event}:${m.matcher}`
