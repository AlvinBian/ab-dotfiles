...
import { detectLocalRepos } from '../repo-detect.mjs'
import { generateInstallPlan } from '../auto-plan.mjs'
 ...
 */
export async function phaseAnalyze({ repos, sources, baseDir }) {
  ...
  const tasks = new Listr(
    [
      ...
      },
      {
        title: '偵測本機路徑',
        task: async (ctx, task) => {
          localPaths = await detectLocalRepos(repos)
          const found = Object.keys(localPaths).length
          task.output = `${found}/${repos.length} 找到`
        },
      },
      {
        title: '開發者畫像',
  ...
  plan._fetchedSources = pipelineResult?.eccFetchResult || null
