                      ...
                      task: async (_, sub) => {
                        ...
                        if (fs.existsSync(templatePath)) {
                          ...
                          try {
                            ...
                            sub.output = 'settings-template.json 格式錯誤，跳過設定部署'
                            return
                          }
                  ...
                  task: (_, subtask) => subtask.newListr([
                    {
                      ...
                      task: async (_, sub) => {
                        if ((plan.ecc?.length ?? 0) > 0 && fetchedSources?.sources?.length > 0) {
                          ...
                          const eccByType = { commands: new Set(), agents: new Set(), rules: new Set() }
                          for (const name of plan.ecc) {
                            const type = eccTypeMap[name.replace('.md', '')] || 'commands'
