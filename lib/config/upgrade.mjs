...
export function detectLegacyInstallation() {
	...
	const projectAgentsInGlobal = existingAgents.filter((a) =>
		LEGACY_PROJECT_AGENTS.includes(a),
	);
	const projectRulesInGlobal = existingRules.filter((r) =>
		LEGACY_PROJECT_RULES.includes(r),
	);
	...
	if (fs.existsSync(hooksPath)) {
		try {
			...
			const hookCount = Object.values(hooks.hooks || {}).reduce(
				(s, m) => s + m.length,
				0,
			);
	...
	const userCustomCommands = existingCommands.filter(
		(c) => !ALL_MANAGED.commands.has(c),
	);
	const userCustomAgents = existingAgents.filter(
		(a) => !ALL_MANAGED.agents.has(a),
	);
	const userCustomRules = existingRules.filter(
		(r) => !ALL_MANAGED.rules.has(r),
	);

	const totalProjectItems =
		projectCommandsInGlobal.length +
		projectAgentsInGlobal.length +
		projectRulesInGlobal.length;
	const needsUpgrade =
		totalProjectItems > 0 ||
		hasOldKkday ||
...
export async function runUpgrade(legacyInfo) {
	...
	if (legacyInfo.totalProjectItems > 0) {
		...
		if (legacyInfo.projectCommandsInGlobal.length) {
			lines.push(
				`   ${idx}.1 Commands（${legacyInfo.projectCommandsInGlobal.length}）`,
			);
		...
		if (legacyInfo.projectAgentsInGlobal.length) {
			lines.push(
				`   ${idx}.2 Agents（${legacyInfo.projectAgentsInGlobal.length}）`,
			);
		...
		if (legacyInfo.projectRulesInGlobal.length) {
			lines.push(
				`   ${idx}.3 Rules（${legacyInfo.projectRulesInGlobal.length}）`,
			);
	...
	if (customCount > 0) {
		lines.push("");
		lines.push(
			`${idx}. ⚠ ${customCount} 個非 ab-dotfiles 的自訂配置（不會刪除）`,
		);
		let ci = 1;
		if (legacyInfo.userCustomCommands.length) {
			lines.push(
				`   ${idx}.${ci} Commands（${legacyInfo.userCustomCommands.length}）`,
			);
		...
		if (legacyInfo.userCustomAgents.length) {
			lines.push(
				`   ${idx}.${ci} Agents（${legacyInfo.userCustomAgents.length}）`,
			);
		...
		if (legacyInfo.userCustomRules.length) {
			lines.push(
				`   ${idx}.${ci} Rules（${legacyInfo.userCustomRules.length}）`,
			);
...
async function doUpgrade(legacyInfo) {
	...
	await Promise.all([
		...
		backupIfExists(
			path.join(CLAUDE_DIR, "keybindings.json"),
			"upgrade/keybindings.json",
		),
	]);
