...
export async function fetchAllSources(sources, techStacks, localDir, onProgress = () => {}) {
  ...
  const results = await Promise.all(sorted.map(async source => {
...
export function buildSyncResult(fetched, selectedNames) {
  ...
  for (const src of allDownloaded) {
    for (const f of src.commands) eccTypeMap[f.name.replace('.md', '')] = 'commands'
    for (const f of src.agents) eccTypeMap[f.name.replace('.md', '')] = 'agents'
    for (const f of src.rules) eccTypeMap[f.name.replace('.md', '')] = 'rules'
  }

  return { results: allResults, downloaded: allDownloaded, eccTypeMap, timestamp: new Date().toISOString() }
}
  ...
  try {
    for (const src of downloaded) {
      for (const f of src.commands) {
        ...
        if (isUserCustomized(dest)) { skipped.push(`commands/${f.name}`); continue }
      ...
      for (const f of src.agents) {
        ...
        if (isUserCustomized(dest)) { skipped.push(`agents/${f.name}`); continue }
        fs.writeFileSync(dest, GENERATED_MARKER + f.content, 'utf8')
        written.push(dest)
      }
      for (const f of src.rules) {
        const dest = path.join(targetDir, 'rules', f.name)
        if (isUserCustomized(dest)) { skipped.push(`rules/${f.name}`); continue }
        fs.writeFileSync(dest, GENERATED_MARKER + f.content, 'utf8')
        written.push(dest)
      }
