...
import { AI_CONCURRENCY } from '../core/constants.mjs'
...
export async function phaseExecute(plan, {
  ...
  const tasks = new Listr([
          ...
          task: async (_, subtask) => {
            if ((plan.techStacks?.length ?? 0) > 0) {
              try {
                ...
                subtask.output = `已生成 ${(plan.techStacks?.length ?? 0)} 個技術棧規則`
              } catch (e) {
    ...
    // [5/8] CLAUDE.md 生成 → ~/.claude/projects/
    {
      ...
      task: async (_, task) => {
        const { pMap } = await import('../core/concurrency.mjs')
        const items = await pMap(plan.projects || [], async (proj) => {
