...
export async function phasePlan(plan) {
  ...
  const kbNames = Object.entries(g.keybindings).map(([k, v]) => `${k}→${v}`)
  lines.push(`   快捷鍵 ${kbNames.join('  ')}`)
  ...
  if (plan.ecc.length > 0) {
    // 按 type 分組（commands/agents/rules）
    const eccByType = { commands: [], agents: [], rules: [] }
    for (const name of plan.ecc) {
      const clean = name.replace('.md', '')
      if (fs.existsSync(path.join(claudeDir, 'commands', `${clean}.md`))) eccByType.commands.push(clean)
      else if (fs.existsSync(path.join(claudeDir, 'agents', `${clean}.md`))) eccByType.agents.push(clean)
      else if (fs.existsSync(path.join(claudeDir, 'rules', `${clean}.md`))) eccByType.rules.push(clean)
      else eccByType.commands.push(clean) // 預設歸到 commands
    }
