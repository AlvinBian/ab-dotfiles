...
function renderStacks(stacks) {
	...
	return section(
		"Generated Stacks",
		`<div>${stacks.map((s) => badge(s, "pink", s)).join("")}</div>`,
	);
}
...
function renderAuditTrail(auditSummary) {
	...
	return section(
		"決策審計鏈",
		'<p class="section-desc">Pipeline 分析過程中的每一步決策記錄，包括 AI 分類結果、整合去重判斷、ECC 匹配邏輯。用於追溯為什麼某個技術棧被選中或排除。</p>' +
...
function renderBackup(backupDir) {
	...
	return section(
		"備份",
		'<p class="section-desc">每次安裝前自動備份現有配置，保留最近 10 次。可用 pnpm run restore 選擇備份版本還原。</p>' +
			`<p class="mono">${esc(backupDir)}</p>`,
	);
}
...
function renderCharts(data) {
	...
	const allCategories = Array.from(
		new Set([
			...
			...Object.values(perRepoReasoning).flatMap((v) =>
				Object.keys(v?.stacks || {}),
			),
		]),
	);
	...
	const catCountsOverview = Object.values(techStacks)
		.map((v) => v?.length || 0)
		.reverse();
...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('resize', function() {
      _charts.forEach(function(c) { try { c.resize(); } catch(e) { /* 圖表元件不支援 resize 則略過 */ } });
    });
...
function renderTabOverview(data) {
	const techStacks = data.techStacks || {};
	const hasCost = Object.values(data.perRepoReasoning || {}).some(
		(v) => v?.cost != null && v.cost > 0,
	);
	const installed = data.installed || {};
	const hasInstall = ["commands", "agents", "rules", "modules"].some(
		(k) => installed[k]?.length,
	);
