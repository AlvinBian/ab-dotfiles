	...
	for (const m of missing) {
		...
		if (m.name === "claude CLI") {
			...
			if (existingPath) {
				...
				// 優先使用官方安裝器（裝到 ~/.local/bin/），fallback npm 全局
				s.start("📦 安裝 Claude CLI（官方安裝器）...");
				let ok = run("curl -fsSL https://claude.ai/install.sh | sh");
				if (!ok || !findClaudeCli()) {
					s.stop("官方安裝器失敗，嘗試 npm...");
					s.start("📦 安裝 Claude CLI（npm）...");
					ok = run("npm install -g @anthropic-ai/claude-code");
				}
				...
				if (!findClaudeCli()) {
					p.log.warn(
						`Claude CLI 安裝失敗，請手動安裝後重新執行：\n` +
							...
							`  ${pc.cyan("npm install -g @anthropic-ai/claude-code")}       # npm 全局\n` +
