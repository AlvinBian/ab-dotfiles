...
export async function phaseExecute(plan, {
  ...
  const tasks = new Listr([
    ...
    {
      title: '[2/7] 全局配置 → ~/.claude/settings.json + keybindings.json',
    ...
    // [3/7] Claude 安裝（commands + agents + rules + hooks → ~/.claude/）
    {
      title: '[3/7] Claude 安裝 → ~/.claude/commands + agents + rules + hooks',
      task: async (ctx, task) => {
    ...
    // [4/7] ECC 外部資源 + 技術棧 Stacks
    {
      title: `[4/7] ECC（${plan.ecc.length} 個外部資源）+ Stacks（${plan.techStacks.length} 個技術棧）`,
      task: (ctx, task) => task.newListr([
        {
    ...
    // [5/7] Plugin 打包
    {
      title: '[5/7] Plugin 打包 → dist/release/',
      task: (ctx, task) => task.newListr([
        {
    ...
    // [6/7] zsh 模組 → ~/.zsh/modules/
    {
      title: `[6/7] zsh 模組（${plan.zshModules.length} 個）→ ~/.zsh/modules/`,
