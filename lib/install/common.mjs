/**
 * 安裝通用工具 — selectItems、buildCmdArgs、showFileList
 */
 ...
 * 顯示已生成的檔案清單
 */
export function showFileList(items, stepLabel) {
  if (items.length === 0) return
  p.log.message(items.map((item, i) =>
    `  ${pc.green('✔')} ${pc.dim(`[${i + 1}/${items.length}]`)} ${item}`
  ).join('\n'))
}
