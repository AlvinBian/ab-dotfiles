...
import { GH_PER_PAGE, DESC_MAX_LENGTH } from '../core/constants.mjs'
...
export async function interactiveRepoSelect(session = null) {
  ...
  if (session?.org && sources.some(s => s.value === session.org)) {
    // 有 session → 自動選擇上次的（可能是單個或多個）
    const prevOrgs = Array.isArray(session.org) ? session.org : [session.org]
  ...
  const ownerQualifier = isPersonal ? `user:${selectedSource}` : `org:${selectedSource}`
  const quickContribRaw = ghSync(
    `search/commits?q=author:${username}+${ownerQualifier}&sort=author-date&per_page=${GH_PER_PAGE}`,
  ...
  // 6. 排序（貢獻度優先）
  const sorted = orderBy(allRepos, ['pct', 'commits'], ['desc', 'desc'])
  ...
  if (selected.length === 0) { p.log.warn('未選擇倉庫'); return BACK }
