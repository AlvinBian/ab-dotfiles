...
export async function phasePlan(plan) {
  ...
  // 專案 CLAUDE.md（按角色生成到 ~/.claude/projects/）
  if (plan.projects.length > 0) {
    ...
    lines.push(`📂 CLAUDE.md → ~/.claude/projects/（${plan.projects.length} 個 repo）`)
    if (mainCount) lines.push(`   ${mainCount} 個 AI 生成（⭐主力）`)
    if (tempCount) lines.push(`   ${tempCount} 個靜態模板（🔄臨時）`)
