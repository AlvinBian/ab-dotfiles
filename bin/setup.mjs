...
async function main() {
	...
	async function runQuickInstall({
		...
		if (!sessionPrev.roles)
			p.log.warn("上次 session 無角色資訊，全部預設為 🔄 臨時");
	...
	if (prev && !flagAll && !flagQuick) {
		...
		if (action === "status") {
			...
			if (claude.extra.length > 0) {
				lines.push(
					pc.dim(`  額外 ${claude.extra.length} 個（非 ab-dotfiles 管理）`),
				);
			}
	...
	if (riskySelected.length > 0) {
		...
		p.log.info(
			`選擇了：${riskySelected.map((f) => `${f}（${hints[f]}）`).join("、")}`,
		);
		const ok = handleCancel(
			await p.confirm({ message: "確認繼續？", initialValue: true }),
		);
		if (ok === BACK || !ok) {
			p.outro("已取消");
			return;
