...
審查前若需確認 repo 位置，執行以下指令動態發現所有本地 git repos：
...
**Vue / TypeScript 合規**：
型別在 `.types.ts` | 無 `any` | Composition API | import 排序 | 無 `console.log`

**PHP 合規**：
SQL prepared statement | 輸入驗證 | 無 `var_dump` | API response 統一 | PII 不入 log
