...
function renderTabRepos(data) {
  ...
  const cards = repoKeys.map(repo => {
    // perRepo Map 用短名（split('/')[1]）作 key，需同時嘗試 fullName 和短名
    ...
    // 專案配置資訊
    const localPath = roleInfo.localPath ? roleInfo.localPath.replace(process.env.HOME, '~') : '未找到'
    const claudeMd = proj ? typeLabel[proj.claudeMdType] || '—' : '—'
    const roleDesc = role === 'main' ? '完整 AI 分析 + 技術棧上下文' : role === 'tool' ? '最小配置' : '精簡模板'
...
export function openInBrowser(filePath) {
  ...
  try { execFileSync(cmd, args); return Promise.resolve() }
