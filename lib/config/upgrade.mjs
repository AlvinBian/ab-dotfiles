...
import { ALL_COMMANDS, ALL_AGENTS, ALL_RULES, LEGACY_PROJECT_COMMANDS, LEGACY_PROJECT_AGENTS, LEGACY_PROJECT_RULES } from './config-classifier.mjs'
...
async function doClean() {
  ...
  if (fs.existsSync(envPath)) {
    ...
    envContent = envContent
      .split('\n')
      .filter(line => !/^(SLACK_NOTIFY_CHANNEL|SLACK_NOTIFY_CHANNEL_NAME|SLACK_NOTIFY_MODE|SLACK_DM_CHANNEL|SLACK_WORKSPACE_URL|SLACK_WEBHOOK_URL|SLACK_NOTIFY_USER_ID)=/.test(line))
      .join('\n')
      .replace(/\n{3,}/g, '\n\n')
