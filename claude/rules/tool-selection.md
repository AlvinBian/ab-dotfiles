...
### 維度 5：前景 vs 背景（Foreground vs Background）

決定每個 Agent 是否需要等待結果：

```
啟動 Agent 前問自己：「我需要它的結果才能繼續嗎？」
│
├─ YES → 前景（foreground，預設）
│  適用：結果是下一步的輸入
│  例：@planner 的方案 → @coder 需要方案才能寫
│  例：@debugger 的根因 → 修復需要知道根因
│
├─ NO，但我有其他獨立工作可以做 → 背景（run_in_background: true）
│  適用：結果有用但非阻塞，主進程可以先做別的事
│  例：@security 掃描時，主進程可以先寫測試
│  例：@reviewer 審查時，主進程可以先準備 changelog
│
└─ NO，且沒有其他工作 → 前景（等結果回來再繼續）
```
...
│
├─ 全部互相獨立？
│  ├─ 都需要結果才能繼續 → 前景並行（單一 message 多個 Agent tool calls）
│  ├─ 都不需要立即結果 → 全部背景（run_in_background: true）
│  └─ 混合 → 關鍵的前景，非關鍵的背景
│
├─ 部分有依賴？
│  └─ 獨立的先並行啟動，依賴前一步的等結果後再啟動
│
└─ 全部串行依賴？
   └─ 逐個執行，不並行
```
...
| 場景 | 前景並行 | 背景 |
|------|---------|------|
| PR 前品質門 | @reviewer + @security + @perf-analyzer | — |
| 寫功能同時掃描 | — | @security（背景掃描） |
| 大型重構分析 | @explorer（多個目錄並行） | — |
| 發版前準備 | @reviewer（前景） | @documenter（背景更新文件） |
...
### 範例 F：「幫我寫這個 API，順便掃一下安全」

分析：
- 意圖：創作 + 審查
- 範圍：模組級
- Context 依賴：寫 API 是主線，安全掃描獨立不阻塞
- 可並行：安全掃描不需要等 API 寫完才開始（可以邊寫邊掃既有代碼）
- 前景 vs 背景：寫 API 是前景（主進程或 @coder），安全掃描是**背景**

**推導**：主進程寫 API（需要即時看結果）+ @security 背景掃描（run_in_background）→ 寫完後查看安全報告，一起修

---

### 範例 G：「改完了，幫我發 PR，同時更新文件」

分析：
- 意圖：部署 + 創作（文件）
- 範圍：PR 流程涉及 commit + push + PR 建立；文件是獨立任務
- Context 依賴：PR 流程需要最新 git 狀態（主進程做），文件更新獨立
- 可並行：文件更新不阻塞 PR 流程
- 前景 vs 背景：PR 是前景（用戶等結果），文件是**背景**

**推導**：主進程走 /pr-workflow（前景，需要互動確認）+ @documenter 背景更新 README → PR 建好後，若文件有改動再追加一個 commit

---

### 範例 H：「重構這個模組，但我不確定影響範圍多大」

分析：
- 意圖：修改（重構），但範圍未知
- 範圍：**先探查再決定**
- Context 依賴：重構依賴影響範圍分析
- 可並行：探查和重構有順序依賴
- 前景 vs 背景：探查是前景（需要結果才能決定下一步）

**推導**：@explorer 前景分析影響範圍 → 結果回來後判斷：
- 若 < 5 檔案 → 主進程直接重構
- 若 5-15 檔案 → @refactor agent
- 若 15+ 檔案 → 先 @planner 拆任務，再分批 @refactor

**關鍵**：不要在不知道範圍時就決定用什麼工具。探查先行。

---

### 範例 I：「這個 API 回應變慢了，幫我查」

分析：
- 意圖：分析 + 可能修復
- 範圍：不確定（可能是代碼、可能是 DB、可能是外部依賴）
- Context 依賴：多個分析維度互相獨立，修復依賴分析結果
- 可並行：**分析階段可以並行，修復必須等分析完**
- 前景 vs 背景：全部前景（需要結果才能決定修什麼）

**推導**：Fan-out 前景並行 @perf-analyzer + @monitor + @data-analyst → 三份報告彙整 → 定位瓶頸 → @debugger 或 @coder 修復

**關鍵**：不要一開始就猜是哪裡慢。多維度同時探查，用結果說話。

---

### 範例 J：「幫我寫 Slack 公告，告訴團隊這次 release 的內容」

分析：
- 意圖：通訊
- 範圍：單點（一則訊息）
- Context 依賴：需要知道 release 內容 → 可能需要讀 git log 或 changelog
- 可並行：否，線性任務
- 前景 vs 背景：前景（用戶要看草稿確認再發）

**推導**：主進程讀 git log / CHANGELOG → /draft-slack 生成草稿 → /review-slack 檢查格式 → 用戶確認後發送

**關鍵**：不需要 Agent。單一輸出物的任務，主進程直接用 Command 就好。
...
- 意圖：審查（多維度）
- 範圍：系統級
- Context 依賴：各審查維度完全獨立
- 可並行：**全部並行**
- 前景 vs 背景：全部前景（要等所有結果彙整才能判斷能不能上線）

**推導**：單一 message 同時啟動 5 個前景 Agent：
- @reviewer（代碼品質）
- @security（安全漏洞）
- @perf-analyzer（效能）
- @typescript-reviewer（型別安全，若 TS 專案）
- @database-reviewer（DB 查詢，若有 DB）

→ 五份報告彙整 → /quality-gate 產出上線判定

**關鍵**：不要一個一個跑。獨立的審查維度永遠並行，省時間。
